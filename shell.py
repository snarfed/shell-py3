"""
An interactive, stateful AJAX shell that runs Python 3 code on the server.

May be run as a standalone app or in an existing app as an admin-only handler.
Can be used for system administration tasks, as an interactive way to try out
APIs, or as a debugging aid during development.

The logging, os, sys, and ndb modules are imported automatically.
...
UNPICKLABLE_TYPES = (
  types.ModuleType,
  types.FunctionType,
  type,  # types themselves and classes
)
  ...
  def get(self):
    with appengine_config.ndb_client.context():
      ...
      if session_key:
        ...
        session_key = session.put().urlsafe().decode()

      template = jinja2.Environment(
        loader=jinja2.FileSystemLoader(('.', 'templates')),
        autoescape=True,
      ).get_template('templates/shell.html')
...
class StatementHandler(webapp2.RequestHandler):
  ...
  def post(self):
    with appengine_config.ndb_client.context():
      ...
      session = ndb.Key(urlsafe=self.request.get('session')).get()
